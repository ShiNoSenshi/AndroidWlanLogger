...
public class WlanMapXmlConverter {
    ...
    private static Location getValue(Node node) {
        ...
        return createLocation(provider, accuracy, altitude, bearing, latitude, longitude, speed, time);
    }

    private static Location createLocation(String provider, float accuracy, double altitude, float bearing,
            double latitude, double longitude, float speed, long time) {
    ...
    private static String getTextValue(Node node, String tagName) {
        Node searchedNode = getNode(tagName, node.getChildNodes());
        return searchedNode.getFirstChild().getNodeValue();
    }
